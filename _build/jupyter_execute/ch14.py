...
# 어느 모임에는 n명의 사람들이 있고, 각자 메시지를 보낼 수 있는 사람과 메시지를 전달하는데 소요되는 시간
# 이 다르다. 
# 
# 
# <div align="center">
#     <img src="https://raw.githubusercontent.com/hj617kim/core_pybook/master/images/ch14/core_img.png" style="width:350px;">
# </div>
# 

# 이때 연결 그래프는 아래와 같이 딕셔너리로 표현한다.
# 
# ```
# graph = {0: [[2, 1]], 1: [[2, 1], [5, 3]], 2: [[1, 3], [3, 4], [4, 1], [5, 2]], 3: [[4, 4]], 
#         4: [[2, 2], [3, 2]], 5: [[2, 9], [4, 7]]}
# ```
# 
# 이는 6명의 사람(0~5)이 있고 예를 들어, 0번 사람은 2번 사람에게 메시지를 보낼 수 있고 1초가 걸린다는 의미다.
# 
# 0번 사람은 2번 사람에게만 메시지를 보낼 수 있지만 2번을 거쳐 1, 3, 4, 5에게 메시지가 전달되게 할 수 있다.
# 메시지를 받은 사람은 보낼 수 있는 모든 사람에게 메시지를 전달한다고 가정할 때, 연결 그래프와 메시지를 보
# 내려는 사람을 인자로 받아 메시지를 받는 사람의 수와 모두 메시지를 받는 데 걸리는 시간을 튜플로 묶어 반환
# 하는 함수 `send_message()`를 우선순위 큐와 다익스트라Dijkstra 알고리즘을 사용해 정의하라.
# 
# 예를 들어, graph의 키 1에 대응하는 값 `[[2, 1], [5, 3]]`은 1번 사람이 메시지를 보낼 수 있는 사람과 그때의 소
# 요시간이다. 즉, 1번 사람은 2번과 5번 사람에게 메시지를 보낼 수 있으며 각각 1초, 3초가 소요된다.
# 
# 1번 사람이 메시지를 보내면 2번, 3번(1번->2번->4번->3번), 4번(1번-2번->4번), 5번 사람이 메시지를 받을 수
# 있고 각각 소요되는 시람은 1초, 4초, 2초, 3초이다.
# 따라서, 1번 사람이 메시지를 보내면 메시지를 받는 사람의 수는 4명이고, 4명은 4초가 지나면 메시지를 전달받
# 는다.
# 
# [참고] 우선순위 큐는 queue 모듈의 PriorityQueue 클래스를 사용한다. 우선순위 큐는 우선순위가 높은 항목
# 을 먼저 꺼내는 데, 우선순위 큐에 항목을 추가할 때는 put() 메서드를, 항목을 꺼낼 때는 get() 메서드를 이용
# 한다.
# [참고] math 모듈의 inf는 양의 무한대를 나타낸다.
