...
# Input :  
# ```
# words = '어떤 고양이는 밥을 먹고 있고 어떤 고양이는 잠을 자고 있으며 강아지는 잠을 자고 있다'
# target_words('는', '을', words = words)   
# ```  
# Output : `고양이(2) 잠(2)`

# ### 문제  
# 진약수를 구하여 합한 값이 자기자신이 되는 양의 정수를 완전수라고 부른다. 이때, 진약수는 자기자신을 제외한 양의 약수를 말한다.  
# 예를 들어, 6의 진약수는 1, 2, 3이고, 이를 합하면 6이므로 6은 완전수다.  
# 
# 양의 정수를 인자로 받아 그보다 작거나 같은 완전수들을 리스트에 담아 반환하는 `perfect_number()` 함수를 정의하라.  
# 
# ```
...
# ### 문제
# 
# 소문자로 작성된 문자열(string)을 인자로 받아 아래 조건을 만족시키는 문자는 대문자로 변경해 반환하는 `auto_capitalize()`를 정의하라.   
# 
# [조건]  
# * 문자열의 첫 문자는 대문자로 변경한다.  
# 예) `hello` --> `Hello` 
# * 느낌표(`!`), 물음표(`?`), 마침표(`.`) 다음에 등장하는 공백이 아닌 문자는 대문자로 변경한다.   
# 예) `hello? python` --> `Hello? Python`  
# 예) `hello?python` --> `Hello?Python`  
# 
# * 소문자 `i`의 경우, 앞뒤에 공백이 있거나 느낌표/물음표/마침표 다음에 등장하는 첫 문자거나 어퍼스트러피(`'`, apostrophe) 앞에 있다면 대문자로 변경한다.   
# 예) `i'm tired` --> `I'm tired`   
# 예) `what do i call you?` --> `What do I call you?`
# 
# 
# 
# 입력값 1
...
# ### 문제
# 
# 자신의 제곱수를 임의의 두 부분으로 나누어 더했을 때 다시 원래의 수가 되는 수를 카프리카 수(Kaprekar number)라고 부른다.   
# 예1) 55는 카프리카 수이다.    
# 55의 제곱은 3025이고, 30 + 25 = 55이다.  
# 
# 예2) 297은 카프리카 수이다.     
# 297의 제곱은 88209이고, 88 + 209 = 297이다.   
# 
# 예3) 13은 카프리카 수가 아니다.  
# 13의 제곱은 169이고, 1 + 69 = 70 != 13이고 16 + 9 = 25 != 13이다.  
# 
# 정수를 인자로 받아 카프리카 수이면 `True` 아니면 `False`를 반환하는 
# 함수 `kaprekar_number()`를 정의하라.   
# 
# 입력값 1  
# ```
# 55
...
# ### 문제
# 
# 8X8의 체스판에 8명의 여왕(Queen)을 아래의 규칙을 만족시키는 장소에 두려고 한다.   
# [규칙]
# * 각 열에는 하나의 여왕만 존재한다. 
# * 각 행에는 하나의 여왕만 존재한다. 
# * 각 여왕의 대각선에 다른 여왕이 올 수 없다. 
# * 여왕은 `o`, 빈장소는 `x`라고 둔다. 
# 
# 예를 들어, 체스판의 일부만 생각했을 때, 아래와 같이 여왕을 배치할 수 없다.  
# 
# <div align="center">
#     <img src="https://raw.githubusercontent.com/hj617kim/core_pybook/master/images/ch06/eight_queens.png" style="width:500px;">
# </div>
# 
# 
# 6명의 여왕이 배치된 체스판을 입력 받아, 나머지 두 여왕의 위치 튜플로 출력하는 코드를 작성하라.
# 
...
# 
# 
# 출력할 때는 아래와 같이 튜플의 0인덱스 값이 작은 것부터 출력한다.    
# 출력값
# ```
# (4, 7)
# (5, 1)
...
# ### 문제
# 
# 카운트(count)와 이터러블(iterable)의 항목을 튜플로 묶은 형태로 이터레이터를 만들어 반환하는 `my_enumerate(iterable, start = 0, is_int = False)`을 정의하고, 각 항목을 꺼내 출력하는 코드를 작성하라. 
# * 카운트는 기본적으로 0부터 시작하고 다른 값부터 시작하고 싶다면 `start` 값을 변경해주면 된다.  
# * `is_int`의 기본값은 `False`다. 만약, `is_int`의 값이 `True`라면 
# 이터러블의 각 항목을 정수로 만든 다음 카운트와 이터러블의 항목을 튜플로 묶은 형태의 이터레이터를 만들면 된다. 
# 
# 입력값은 3줄로 들어오며, 첫 줄은 공백으로 분리되어 입력된다. 둘째 줄은 `start` 값이고, 셋째 줄은 `is_int`의 값이다.
# 
# [주의! 1점] `my_enumerate(map(int, '123'), start = 0, is_int = False)` 도 동작해야 한다.
# 
# 입력값 1
# ```
# 봄 여름 가을 겨울
