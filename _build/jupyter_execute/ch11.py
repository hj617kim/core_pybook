...
# $w$kg까지 넣을 수 있는 가방을 들고 쥬얼리샵에 침입하였다고 가정한다. 훔칠 수 있는 `n`개의 보석이 주어졌고, 각 보석은 서로 다른 무게를 갖는다고 가정한다. 가방에 넣은 보석의 값어치가 최대가 되도록 할 때, 가방에 넣은 보석의 종류와 가격을 반환하는 함수`bag1(stuff, w)`를 정의하여라.  
# 
...
# $w$kg까지 넣을 수 있는 가방을 들고 쥬얼리샵에 침입하였다고 가정한다. 훔칠 수 있는 `n`개의 보석가루가 주어졌고, 각 보석 가루는 서로 다른 무게를 갖는다고 가정한다. 가방에 넣은 보석 가루의 값어치가 최대가 되도록 할 때, 가방에 넣은 보석 가루의 종류와 가격을 반환하는 함수`bag2(stuff, w)`를 정의하여라.  
# 
# 예를 들어, 5개의 보석이 있고, 각각의 가격과 무게가 아래 표와 같다면, `stuff = [[2, 3], [3, 4], [4, 8], [5, 8], [9, 10]]`이다.  
# 
# |보석류|무게|가격|
