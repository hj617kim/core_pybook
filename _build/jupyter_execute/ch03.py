...
# * `name`  
#   
# 변수명은 알파벳 문자나 밑줄기호로 시작해야 하므로 `123k`는 변수명으로 사용할 수 없고, `!` 기호도 변수명에 사용될 수 없다.  
# 위에서 가능한 변수명은 `_K1212`와 `name`이다.  
# 
# ````
...
# ### 문제   
# 오늘이 무슨 요일이 주어질 때, 100일 후는 무슨 요일인지를 출력하는 코드를 작성하여라.  
...
# ### 문제  
# 30cm × 30cm 크기의 타일이 있다. 타일은 온장을 그대로 사용할 수도 있고, 잘라서 일부분만 사용할 수도 있다. **단, 잘라서 사용한 타일의 나머지는 사용하지 않는다.**   
# 주어진 크기의 바닥에 타일을 깔기 위해 사용되는 온장 타일과 잘라서 사용한 타일의 개수를 공백으로 구분하여 출력하는 코드를 작성하여라.  
# 
...
# ### 문제  
# 
# 어느 대기실에는 N개의 의자가 일렬로 놓여 있다. 어느 두 사람도 이웃하여 앉지 않을 때, 앉을 수 있는 최대 인원수와 최소 인원수를 출력하는 코드를 구하여라.  
# 예를 들어, 5개의 의자가 일렬로 놓여 있을 때, 최대 3명(ex, ■□■□■)이 앉을 수 있고, 최소 2명(ex, ■□□■□)이 앉을 수 있다.  
