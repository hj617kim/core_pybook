...
# > `2personalinformation`  
# 
# 문서 이름 리스트를 입력 받아, 문서 제목의 알파벳 순서대로 정렬된 리스트를 반환하는 `docu_sort()`함수를 정의하여라. 단, 문서 제목은 모두 영어로 되어 있고, 문서 정렬은 역순문서제목은 무시하고 문서 제목만을 기준으로 삼으며, 문서 제목이 같은 경우 버전을 기준으로 오름차순 정렬한다.   
# 
# ```python
...
# 어린이 합창단 A와 B는 마지막 곡을 함께 부르기로 했다. 두 합창단은 키 순서대로 무대에 입장하는 데, 마지막 곡을 부르기 위해 키를 기준으로 두 줄로 선 합창단 A와 B는 한 줄로 서야 한다. 합창반 선생님은 다음과 같은 방식으로 두 줄을 한 줄로 합치려고 한다.   
# 
...
# 
# A와 B 합창단원의 이름과 키가 딕셔너리 형태로 주어졌을 때, 두 딕셔너리를 입력 받아 줄이 합쳐지는 과정을 보여주는 `merge_chorus()`함수를 정의하여라.   
# 
# ```python
...
# 
# 임의로 서 있는 번호 리스트와 사은품의 개수를 입력 받아, 위의 방법대로 정렬된 리스트와 마지막으로 사은품을 수령하는 번호를 반환하는 `event_sort()` 함수를 정의하여라.
# 
# ```python
...
# 어구전철<font size="2">anagram, 애너그램</font>은 문자의 순서를 변경하여 다른 단어나 문장을 만드는 놀이로, 때로는 암호로 사용하기도 한다. 예를 들어, '주문' - '문자', 'listen' - 'silent', 'TOM MARVOLO RIDDLE' - 'I AM LORD VOLDEMORT' 등이 있다.   
# 
