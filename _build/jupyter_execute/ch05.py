...
# ### 문제  
# 주어진 빙고판에서 왼쪽 상단에서 오른쪽 하단을 연결한 대각선(`\`) 빙고의 총 개수를 세는 프로그램을 만들어보자. 빙고판의 행의 개수와 열의 개수는 모두 2이상이고, 행의 기수와 열의 개수는 같지 않을 수도 있다. 빙고 여부는 `'o'`기준으로 생각하며 빙고를 만족하기 위해 나란히 있어야 하는 `'o'`의 개수는 min(행의 개수, 열의 개수)이다.   
...
# * 행의 개수가 열의 개수보다 작은 경우. 아래 예제에서는 (0, 0), (1, 1), (2, 2)와 (0, 1), (1, 2), (2, 3) 그리고 (0, 2), (1, 3), (2, 4) 위치의 값만 확인하면 됨. 2빙고
# 
# ```
# >>> bingo_board  = [['o', 'o', 'o'],
#                     ['x', 'o', 'x'],
